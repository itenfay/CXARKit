...
#if os(macOS)
public typealias SCNFont = NSFont
#else
public typealias SCNFont = UIFont
#endif
...
public class SCNTextPainter: NSObject {
    ...
    private func createBubbleParentNode(_ s: String) -> SCNNode {
        ...
        var bubbleFont: SCNFont? = nil
        let descriptor = font?.fontDescriptor.withSymbolicTraits(.traitBold)
        #if os(macOS)
        bubbleFont = SCNFont(descriptor: descriptor, size: 0) ?? SCNFont.systemFont(ofSize: 16)
        #else
        bubbleFont = SCNFont(descriptor: descriptor!, size: 0)
        #endif
        // BUBBLE-TEXT

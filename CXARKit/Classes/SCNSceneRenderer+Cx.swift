...
#if os(macOS)
public typealias SCNImage = NSImage
#else
public typealias SCNImage = UIImage
#endif
...
extension SCNSceneRenderer {
    ...
    public func cx_snapshot(for scnView: SCNView) -> SCNImage {
        #if os(macOS)
    ...
    /// var renderer: SCNSceneRenderer
    /// Needs to set renderer.delegate = `target`
    public func cx_snapshot(for scnView: SCNView, size: CGSize) -> SCNImage {
